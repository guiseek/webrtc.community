...
describe('CookieStorage', () => {
  it('should create an instance', () => {
    expect(new CookieStorage()).toBeTruthy()
  })
})
...
describe('NgxCookieStorageService', () => {
  ...
  beforeEach(() => {
    documentCookieGetterSpy = spyOnProperty(
      documentMock,
      'cookie',
      'get'
    ).and.callThrough()
    documentCookieSetterSpy = spyOnProperty(
      documentMock,
      'cookie',
      'set'
    ).and.callThrough()
    ...
    cookieService = TestBed.inject(CookieStorage)
  })
  ...
  describe('Platform browser', () => {
    ...
    describe('#check', () => {
      ...
      it('should look up cookie by encoded name', () => {
        documentCookieGetterSpy.and.returnValue(
          '%3B%2C%2F%3F%3A%40%26%3D%2B%24=exists;'
        )
    ...
    describe('#set', () => {
      ...
      it('should set cookie with encoded name and value', () => {
        ...
        expect(documentCookieSetterSpy).toHaveBeenCalledWith(
          '%24uper%5ETEST(123)=%24uper%5ETEST(123);sameSite=Lax;'
        )
        expect(documentCookieSetterSpy).toHaveBeenCalledWith(
          'F()!!()%2Fbar=F()!!()%2Fbar;sameSite=Lax;'
        )
        expect(documentCookieSetterSpy).toHaveBeenCalledWith(
          '*F.)%2Fo(o*=*F.)%2Fo(o*;sameSite=Lax;'
        )
        expect(documentCookieSetterSpy).toHaveBeenCalledWith(
          '-O_o%7B1%2C2%7D=-O_o%7B1%2C2%7D;sameSite=Lax;'
        )
        expect(documentCookieSetterSpy).toHaveBeenCalledWith(
          'B%3Far%7CFo%2Bo=B%3Far%7CFo%2Bo;sameSite=Lax;'
        )
        expect(documentCookieSetterSpy).toHaveBeenCalledWith(
          'Hello%3DWorld%3B=Hello%3DWorld%3B;sameSite=Lax;'
        )
        expect(documentCookieSetterSpy).toHaveBeenCalledWith(
          '%5Bfoo-_*.%5Dbar=%5Bfoo-_*.%5Dbar;sameSite=Lax;'
        )
      })
      ...
      it('should set cookie with expires option in options body', () => {
        ...
        expect(documentCookieSetterSpy).toHaveBeenCalledWith(
          'foo=bar;expires=Tue, 01 Sep 2020 10:00:00 GMT;sameSite=Lax;'
        )
        jasmine.clock().uninstall()
      })
      ...
      it('should set cookie with path option', () => {
        ...
        expect(documentCookieSetterSpy).toHaveBeenCalledWith(
          'foo=bar;path=/test;sameSite=Lax;'
        )
      })
      ...
      it('should set cookie with domain option', () => {
        ...
        expect(documentCookieSetterSpy).toHaveBeenCalledWith(
          'foo=bar;domain=example.com;sameSite=Lax;'
        )
      })
