...
export class NavFocusService implements OnDestroy {
  ...
  relinquishSkipLinkFocus(el: HTMLElement) {
    ...
    const skipLinkFocusTarget = this.skipLinkFocusRequests[
      this.skipLinkFocusRequests.length - 1
    ]
  ...
  isNavWithinComponentView(previousUrl: string, newUrl: string) {
